...
envelope) and serves whatever domains are registered via `RouteRegistrar` —
it never imports a domain package.

## What's here

- `server.go` — `Server`, `NewServer`, and the `RouteRegistrar`/`Option`
  ...
  `RouteRegistrar` per domain (e.g. `internal/item/http.Handler`). `Start`
...
- `config.go` — `Config`, the server's tunables (port, timeouts, body
  limit, env, version), sourced from `internal/config` by the composition
  root.
...
`RouteRegistrar` in the domain's own `http/` subpackage and pass it to
